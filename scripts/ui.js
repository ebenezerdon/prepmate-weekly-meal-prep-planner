...
;(function (win, $) {
	...
	function renderDishes() {
		...
		if (!State.dishes.length) {
			$list.append(
				'<div class="text-sm text-gray-500">No dishes yet. Add one!</div>'
			)
			return
		}
		State.dishes.forEach((d) => {
			...
			const $left = $(
				`<div class="flex items-center gap-3 flex-1 min-w-0"></div>`
			)
			$left.append(
				$(
					`<div style=\"width:36px;height:36px;border-radius:8px;background:${
						d.color
					};color:white;display:flex;align-items:center;justify-content:center;font-weight:700;\">${d.name.charAt(
						0
					)}</div>`
				)
			)
			$left.append(
				$(
					`<div><div class=\"font-medium\">${d.name}</div><div class=\"text-xs text-gray-500\">Makes ${d.servings} servings</div></div>`
				)
			)
			const $right = $(
				`<div class="flex items-center gap-2 flex-shrink-0 whitespace-nowrap"></div>`
			)
			const $use = $(
				`<button class=\"btn-muted\" data-id=\"${d.id}\">Use</button>`
			)
			$use.on("click", () => openAssignForDish(d.id))
			const $del = $(
				`<button class="btn-muted text-rose-600" type="button">Delete</button>`
			)
			$del.on("click", () => openDeleteModalForDish(d.id))
	...
	function renderWeek() {
		...
		DAYS.forEach((d) => {
			$trh.append(`<th class="p-2 text-sm text-left">${d}</th>`)
		})
		$thead.append($trh)
		$table.append($thead)
		...
		MEALS.forEach((meal) => {
			...
			DAYS.forEach((day) => {
				...
				const $td = $(
					`<td class="p-2 align-top"><div class=\"slot\" data-key=\"${key}\"></div></td>`
				)
				...
				if (entry && entry.dishId) {
					...
					if (dish) {
						...
						const pill = $(
							`<div class=\"dish-pill\" style=\"background:${dish.color};\">${dish.name}<span style=\"font-size:11px;opacity:0.9;margin-left:8px;font-weight:600;\">×${used}</span></div>`
						)
						$slot.append(pill)
						...
						if (remaining > 0) {
							$slot.append(
								`<div class=\"dish-leftover mt-2\">${remaining} leftover(s)</div>`
							)
						}
					} else {
						$slot.append(
							'<div class="text-sm text-gray-500">(missing dish)</div>'
						)
					}
				} else {
	...
	function computeUsedForDish(dishId) {
		let total = 0
		Object.values(State.plan || {}).forEach((e) => {
			if (e && e.dishId === dishId) total += e.servings || 1
		})
		return total
	}
	...
	function renderSummary() {
		...
		if (!left.length) {
			$leftList.text("No leftovers")
		} else {
			left.forEach((l) => {
				$leftList.append(`<div>${l.name} — ${l.remaining} leftover(s)</div>`)
			})
		}
		// servings summary
		const totalPlanned = Object.values(State.plan).reduce(
			(s, e) => s + (e && e.servings ? e.servings : 0),
			0
		)
	...
	function removeDish(id) {
		...
		Object.keys(State.plan).forEach((k) => {
			if (State.plan[k] && State.plan[k].dishId === id) delete State.plan[k]
		})
		syncToStorage()
		renderAll()
	}
	...
	function openDeleteModalForDish(dishId) {
	...
	function populateAssignDishSelect(preselectId) {
		...
		State.dishes.forEach((d) => {
			const opt = $(
				`<option value=\"${d.id}\">${d.name} (makes ${d.servings})</option>`
			)
			DOM.assignDish.append(opt)
		})
		if (preselectId) {
			DOM.assignDish.val(preselectId)
		}
	}
	...
	win.App.init = function () {
		...
		// event wiring
		DOM.addDishBtn.on("click", () => {
			DOM.dishForm.show()
			DOM.dishForm.attr("aria-hidden", "false")
		})
		DOM.cancelDish.on("click", () => {
			DOM.dishForm.hide()
		...
		DOM.toggleAuto.on("change", function () {
			State.settings.autoBalance = $(this).is(":checked")
			syncToStorage()
			applyAutoBalanceIfNeeded()
			renderAll()
		})
		...
		DOM.assignModal.show = function () {
			DOM.assignModal.removeClass("hidden").css("display", "flex")
			DOM.assignModal.attr("aria-hidden", "false")
		}
		DOM.assignModal.hide = function () {
			DOM.assignModal.addClass("hidden").css("display", "none")
			DOM.assignModal.attr("aria-hidden", "true")
		}

		DOM.deleteModal.show = function () {
			DOM.deleteModal.removeClass("hidden").css("display", "flex")
			DOM.deleteModal.attr("aria-hidden", "false")
		}
		DOM.deleteModal.hide = function () {
			DOM.deleteModal.addClass("hidden").css("display", "none")
		...
		DOM.deleteConfirm.on("click", () => {
			const id = DOM.deleteModal.data("dishId")
			if (id) removeDish(id)
			DOM.deleteModal.hide()
		})
